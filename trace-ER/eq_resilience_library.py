...
def get_Parameters(latitude, longitude):
    city, country_code = coordinate_check(latitude, longitude)
    with open(base_dir +'/hazard_data/countries.json') as countries_file:
        ...
        if country_code in country_codes:
            ...
            ParaTR = get_ParaTR(longitude, latitude, hazard_data)
        ...
        return ParaTR, city, country_code
            ...
            # raise ValueError(f"Earthquake hazard calculation is not supported in {country_code}. Please add Italian coordinates.")
...
def get_map(latitude, longitude, city, country):
    ...
    # print("{}, {}".format(city, country_code))
...
def country_check(latitude, longitude):
    coordinates = (latitude, longitude)
    location = rg.search(coordinates)
    country = location[0]['cc']
    name = location[0]['name']
    hazard_path = os.path.join(base_dir, 'hazard_data', 'HazardNTCgrid_{}.mat'.format(country))    
    if os.path.exists(hazard_path):
        return country
    else:
        raise ValueError(f"Earthquake hazard calculation is not supported in {country}. Please add Italian coordinates.")


def get_data_for_2d_layout_archive(xlsx):
    ...
    n_walls_per_floor = []
    for k in range(N):
        dimensions.append(np.array([xlsx.parse(N-k)['L X (m)'].values, xlsx.parse(N-k)['L Y (m)'].values]).T)
        center_coordinates.append(np.array([xlsx.parse(N-k)['BX (m)'].values, xlsx.parse(N-k)['BY (m)'].values]).T)
...
def get_data_from_description_sheet_archive(xlsx):
    N = xlsx.parse(0).values.shape[0]
    piani = np.array(list(reversed(xlsx.parse(0)['Floors'].values)))
    alt = np.array(list(reversed(xlsx.parse(0)['H'].values)))
    alt_s = np.zeros(N)
...
def get_data_from_floor_sheets(xlsx, N):
    D = []
    X = []
    S = []
    V = []
...
def get_data_from_floor_sheets_archive(xlsx, N):
    D = []
    X = []
    S = []
    V = []
    ...
    for k in range(N):
        D.append(np.array([xlsx.parse(N-k)['L X (m)'].values, xlsx.parse(N-k)['L Y (m)'].values]).T)
        X.append(np.array([xlsx.parse(N-k)['BX (m)'].values, xlsx.parse(N-k)['BY (m)'].values]).T)
        S.append(xlsx.parse(N-k)['s (t/mq)'].values)
        V.append(xlsx.parse(N-k)['H (m)'].values)
        alpha.append(xlsx.parse(N-k)['α'].values)
        mud.append(xlsx.parse(N-k)['m'].values)
        G.append(np.array([xlsx.parse(N-k)['G'].values, xlsx.parse(N-k)['t (t/mq)'].values, 
                        xlsx.parse(N-k)['E'].values, xlsx.parse(N-k)['fd (t/mq)'], xlsx.parse(N-k)['γ(t/mc)']]).T)
        d.append(xlsx.parse(N-k)['m'].values)
        NZ.append(len(D[k]))
    return D, X, S, V, alpha, mud, G, d, NZ


def get_current_data(N, D, mud, S, G, V, NZ, alpha, check):
    dult = []
    dult_drift = []
...
def get_Vr_and_Tr(service_life, class_str):
    match class_str:
        case 'I':
            cu=0.7
        case 'II':
...
def get_ParaTR(longitude, latitude, hazard_data):
...
def triterazioneapp2_solotreparam(ParaTR, Tr, Trmax=2475, table=None):
    if table is None:
...
def get_Sgeo_and_Cc(soli_category, Parametri):
    match soli_category:
...
def get_S_t(topographic_category):
...
def get_ADRS(TC, Parametri, S_geo):
    TB=TC/3
    TD=4*Parametri[0]+1.6

    tstep = get_tstep(TC, TB, TD)
    ntstep = len(tstep)
...
def compute_paretisingole_figapp_archive(D,S,V,G,N,NZ,alpha,hTOT,alt,Parametri,q,S_geo):
    T=(hTOT**(3/4))*0.05
    q_ort=3
    gamma_m=2
    ag_SLV=Parametri[0]
    ...
    for j in range(N):
        ...
        for i in range(NZ[j]):
            ...
            if D0[i,0]>D0[i,1]:
                ...
                Mrd0[i]=((D0[i,1]**2)*D0[i,0]*S0[i]/2)*(1-S0[i]/(0.85*G0[i,3]/gamma_m))
                Mrd0_ort[i]=((D0[i,0]**2)*D0[i,1]*S0[i]/2)*(1-S0[i]/(0.85*G0[i,3]/gamma_m))
...
def compute_paretisingole_figapp(D,S,V,G,N,NZ,alpha,hTOT,alt,Parametri,q,S_geo):
    T=(hTOT**(3/4))*0.05
    q_ort=3
    gamma_m=2
    ag_SLV=Parametri[0]
...
def linear_static_analysis_archive(xlsx, longitude, latitude, soil_category, topographic_category, service_life, importance_class, q):
    country_code = country_check(latitude, longitude)
    hazard_path = 'eq_resiliance_data/HazardNTCgrid_{}.mat'.format(country_code)
    hazard_data = loadmat(hazard_path)['lonlatsism']
    
    #-------- Extracting data from sheet ------------#
    N, _, alt, _, hTOT, _, _ = get_data_from_description_sheet(xlsx)
    D, _, S, V, alpha, _, G, _, NZ = get_data_from_floor_sheets(xlsx, N)

    #----------- Precalculations -----------------#
    _, TrSLV = get_Vr_and_Tr(service_life, importance_class)
    ParaTR = get_ParaTR(longitude, latitude, hazard_data)
    Parametri = triterazioneapp2_solotreparam(ParaTR, TrSLV)
    S_Sgeo, _ = get_Sgeo_and_Cc(soil_category, Parametri)
    S_t = get_S_t(topographic_category)

    S_geo=S_t*S_Sgeo

    IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y = compute_paretisingole_figapp_archive(D,S,V,G,N,NZ,alpha,hTOT,alt,Parametri,q,S_geo)
    return IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y
...
def linear_static_analysis(xlsx, ParaTR, soil_category, topographic_category, service_life, importance_class, q):    
    ...
    IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y = compute_paretisingole_figapp(D,S,V,G,N,NZ,alpha,hTOT,alt,Parametri,q,S_geo)
    return IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y



def calculate_global_boundaries(center_coordinates, dimensions):
    """
...
def plot_floor_plan_archive(center_coordinates_f, dimensions_f, alpha_f, floor_id, x_min, x_max, y_min, y_max, direction=None, failure_mechanism=None, array=None):
    """
    Plot the floor plan for a specific floor with walls and labels.
    """
    fig, ax = plt.subplots(figsize=(12, 8))  # Increase figure size
    title = f'Wall Resistance Floor{floor_id + 1}'
    ...
    for idx, ((cx, cy), (w, h), alpha_i) in enumerate(zip(center_coordinates_f, dimensions_f, alpha_f)):
        ...
        # Color selection based on alpha value
        color = 'lime' if alpha_i >= 45 else 'blue'
        ...
        if array is not None and len(array) > 0:
            if len(array.shape) == 2:
                if idx in dictionary:
                    ...
                    if dictionary[idx]<1:
                        color = 'red'
                        ax.text(cx, lower_left_y -0.75, f'{dictionary[idx]:.2f}', color='red', fontsize=10, ha='center', va='bottom', fontweight='bold')
                    title = fr"Earthquake {direction} - Floor{floor_id + 1} - {failure_mechanism} - $IR_{{min}}={IR_min:.2f}$"
            else:
                if idx in array:
                    color = 'red'
        ...
        # Create and add rectangle for the wall
        rect = patches.Rectangle((lower_left_x, lower_left_y), w, h, edgecolor='black', facecolor=color, linewidth=0.5)
        ax.add_patch(rect)

        # Determine text position based on alpha value
        if alpha_i < 45 or alpha_i >= 135:
            text_x, text_y = cx, cy + h / 2 - 0.05  # Above the wall
            ha, va = 'center', 'bottom'
        else:
            text_x, text_y = cx + w / 2 + 0.08, cy  # To the right of the wall
            ha, va = 'left', 'center'

        # Draw the label (wall number)
        ax.text(text_x, text_y, str(idx + 1), color='black', fontsize=10, ha=ha, va=va, fontweight='bold')
...
def plot_floor_plan(center_coordinates_f, dimensions_f, alpha_f, floor_id, x_min, x_max, y_min, y_max, direction=None, failure_mechanism=None, array=None):
    """
    Plot the floor plan for a specific floor with walls and labels.
    """
    ...
    #                     # else:
    #                     #     ax.text(cx, lower_left_x -0.75, f'{dictionary[idx]:.2f}', color='red', fontsize=12, ha='center', va='bottom', fontweight='bold')
    ...
    #     other_centers = np.concatenate((center_coordinates_f[:idx, :], center_coordinates_f[idx+1:, :]))
    ...
    #             # ha, va = 'center', 'bottom'
    #         else:
    #             text_x, text_y = cx, cy#cx + h / 2 + 0.08, cy  # To the right of the wall
    #             # ha, va = 'left', 'center'
    #         ha, va = 'center', 'center'
    #     else:
    #         if alpha_i < 45 or alpha_i >= 135:
    ...
    #             ha, va = 'center', 'bottom'
    #         else:
    #             text_x, text_y = cx + h / 2 + 0.08, cy  # To the right of the wall
    #             ha, va = 'left', 'center'
    #         # ha, va = 'center', 'center'
...
def plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction=None, failure_mechanism=None, array=None, chosen_floor=None):
    """
    ...
    x_min, x_max, y_min, y_max = calculate_global_boundaries(center_coordinates, dimensions)
    ...
    if chosen_floor is None:
        ...
        for floor in range(N):
            ...
            floor_id = N-floor-1
            if array is not None:
                fig = plot_floor_plan(center_coordinates[floor], dimensions[floor], alpha[floor], floor_id, x_min, x_max, y_min, y_max, direction, failure_mechanism, array[floor])
            else:
                fig = plot_floor_plan(center_coordinates[floor], dimensions[floor], alpha[floor], floor_id, x_min, x_max, y_min, y_max)
            figures.append(fig)
        return figures
    else:
        if chosen_floor-1 >= N:
        ...
        if array is not None:
            fig = plot_floor_plan(center_coordinates[floor_id], dimensions[floor_id], alpha[floor_id], chosen_floor, x_min, x_max, y_min, y_max, direction, failure_mechanism, array[floor_id])
        else:
            fig = plot_floor_plan(center_coordinates[floor_id], dimensions[floor_id], alpha[floor_id], chosen_floor, x_min, x_max, y_min, y_max)
    return fig
    
def plot_wall_resistance_archive(center_coordinates, dimensions, alpha, N, direction=None, failure_mechanism=None, array=None, chosen_floor=None):
    """
    Main function to calculate boundaries and plot the wall resistance for a specific floor.
    """
    # Calculate global boundaries for all floors
    ...
    if chosen_floor is None:
        ...
        for floor in range(N):
            ...
            floor_id = N-floor-1
            if array is not None:
                fig = plot_floor_plan_archive(center_coordinates[floor], dimensions[floor], alpha[floor], floor_id, x_min, x_max, y_min, y_max, direction, failure_mechanism, array[floor])
            else:
                fig = plot_floor_plan_archive(center_coordinates[floor], dimensions[floor], alpha[floor], floor_id, x_min, x_max, y_min, y_max)
            figures.append(fig)
        return figures
    else:
        if chosen_floor-1 >= N:
        ...
        floor_id = N-chosen_floor-1
        if array is not None:
            fig = plot_floor_plan_archive(center_coordinates[floor_id], dimensions[floor_id], alpha[floor_id], chosen_floor, x_min, x_max, y_min, y_max, direction, failure_mechanism, array[floor_id])
        else:
            fig = plot_floor_plan_archive(center_coordinates[floor_id], dimensions[floor_id], alpha[floor_id], chosen_floor, x_min, x_max, y_min, y_max)
    return fig



def plot_wall_failures_archive(xlsx, direction, walls, chosen_floor=None):
    if direction == 'X':
        array = walls[0]
    else:
        array = walls[1]
    ...
    figures = plot_wall_resistance_archive(center_coordinates, dimensions, alpha, N, direction, None, array, chosen_floor)
    return figures
...
def plot_wall_failures(xlsx, direction, walls, chosen_floor=None):
    ...
    figures = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, None, array, chosen_floor)
    return figures
...
def choose_linear_analysis_plot(xlsx, direction, IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y, chosen_floor=None):
    ...
    figures = []
    if direction == 'X':
        fig1 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Shear', IR_fess_X, chosen_floor)
        fig2 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Bending in plane', IR_pf_X, chosen_floor)
        fig3 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Bending out of plane', IR_pf_ort_X, chosen_floor)
    else:
        fig1 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Shear', IR_fess_Y, chosen_floor)
        fig2 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Bending in plane', IR_pf_Y, chosen_floor)
        fig3 = plot_wall_resistance(center_coordinates, dimensions, alpha, N, direction, 'Bending out of plane', IR_pf_ort_Y, chosen_floor)
...
def choose_linear_analysis_plot_archive(xlsx, direction, failure_mechanism, IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y, chosen_floor=None):
    match failure_mechanism:
        case 'Shear':
            if direction == 'X':
                array = IR_fess_X
            else:
        ...
        case 'Bending in plane':
            if direction == 'X':
                array = IR_pf_X
            else:
                array = IR_pf_Y

        case 'Bending out of plane':
            if direction == 'X':
                array = IR_pf_ort_X
            else:
                array = IR_pf_ort_Y
    ...
    figures = plot_wall_resistance_archive(center_coordinates, dimensions, alpha, N, direction, failure_mechanism, array, chosen_floor)
    return figures
...
def get_linear_dataframes(direction, IR_fess_X, IR_fess_Y, IR_pf_X, IR_pf_Y, IR_pf_ort_X, IR_pf_ort_Y, chosen_floor):
    ...
    # for i in range(n):
    #     values = np.concatenate((IR_fess_X[i][:, 0].reshape(-1, 1).astype(np.int64) +1, IR_fess_X[i][:, 1].reshape(-1, 1), IR_pf_X[i][:, 1].reshape(-1, 1), IR_pf_ort_X[i][:, 1].reshape(-1, 1)), axis=1)
    #     columns = ['Wall ID', 'IR (Shear)', 'IR (Bending in plane)', 'IR (Bending out of plane)']
    #     # index = IR_fess_X[i][:, 0]
    #     dataframe = pd.DataFrame(values, columns=columns)
    #     dataframe.columns.name = 'Direction X, floor {}'.format(n-i)
    #     dataframes.append(dataframe)
    # n = len(IR_fess_Y)
    # for i in range(n):
    #     values = np.concatenate((IR_fess_Y[i][:, 0].reshape(-1, 1).astype(np.int64), IR_fess_Y[i][:, 1].reshape(-1, 1), IR_pf_Y[i][:, 1].reshape(-1, 1), IR_pf_ort_Y[i][:, 1].reshape(-1, 1)), axis=1)
    #     columns = ['Wall ID', 'IR (Shear)', 'IR (Bending in plane)', 'IR (Bending out of plane)']
    #     # index = IR_fess_X[i][:, 0]
    #     dataframe = pd.DataFrame(values, columns=columns)
    #     dataframe.columns.name = 'Direction Y, floor {}'.format(n-i)
    #     dataframes.append(dataframe)
...
def compute_IR_deltau_7_fig3app_archive(D,X,S,V,G,deltau,N,NZ,Masse,alt_s,red_F,ADRS,inc,deltared,piani,alpha,passo,Parametri,ParaTR,tstep,S_geo,TC,terreno,algorithm):
    ...
    for j in range(N):
        ...
        for i in range(NZ[j]):
            ...
            if D0[i,0]>D0[i,1]:                      # Alpha check
                ...
                K0[i,0]=Gvar1[i,0]*D0[i,0]*D0[i,1]/(1.2*V0[i]*(1+1/(1.2*Gvar0)*(V0[i]/D0[i,1])**2))*np.sin(theta)**2
                K0[i,1]=Gvar1[i,0]*D0[i,0]*D0[i,1]/(1.2*V0[i]*(1+1/(1.2*Gvar0)*(V0[i]/D0[i,1])**2))*np.cos(theta)**2
...
def compute_IR_deltau_7_fig3app(D,X,S,V,G,deltau,N,NZ,Masse,alt_s,red_F,ADRS,inc,deltared,piani,alpha,passo,Parametri,ParaTR,tstep,S_geo,TC,terreno,algorithm):
...
def get_data_for_plot_2(ADRS, Parametri, Masse, vr_ult_TOT, Hult_TOT, delta_ult_eq, Hult_eq, kult_TOT, tstep, S_geo, TC, ParaTR, soil_category):
    e_point = []
    k_el_TOT = []
    Saa = []
    Sda = []
    ...
    Tr_x, ag_x_TR, ADRS_x_TR, Sda_x_TR, Saa_x_TR = triterazioneapp2(MTOT,Hult_eq[0],S_geo,delta_ult_eq[0],k_el_TOT[0],tstep,ParaTR,soil_category)
    Tr_y, ag_y_TR, ADRS_y_TR, Sda_y_TR, Saa_y_TR = triterazioneapp2(MTOT,Hult_eq[1],S_geo,delta_ult_eq[1],k_el_TOT[1],tstep,ParaTR,soil_category)
    Tr = [Tr_x, Tr_y]
...
def triterazioneapp2(Masse,F_x_eq,S_geo,delta_x_eq,kel_x,tstep,ParaTR,soil_category):
    Trmax=2475
    ...
    for i in range(n):
        ...
        Tr=years[i]
        Parametri0 = triterazioneapp2_solotreparam(ParaTR, Tr)
...
def get_triangle_area(slope, x_curve):
...
def get_triangle_sides(slope, area):
    h = np.sqrt(area*2*slope)
    w = h / slope
    return w, h

def get_bilinear_coordinates(x_curve, y_curve, triangle_height, triangle_width):
...
def calculate_bilinears(X, Y):
    ...
    for i in range(2):
        ...
        curve_area = get_area_under_curve(x, y)
        slope = get_slope(x, y)
        triangle_area = get_triangle_area(slope, x)
        diff_area = triangle_area - curve_area
        w, h = get_triangle_sides(slope, diff_area)
        x_coord, y_coord = get_bilinear_coordinates(x, y, h, w)
...
def get_bilinear_pushover_plot(vr_ult_TOT, Hult_TOT, bilinear_x, bilinear_y):
...
def pushover_analysis_calculation(xlsx, ParaTR, soil_category, topographic_category, service_life, importance_class, check_type):
    #-------- Extracting data from sheet ------------#
    n_floors, piani, _, alt_s, _, Masse, red_F = get_data_from_description_sheet(xlsx)
    D, X, S, V, alpha, mud, G, d, NZ = get_data_from_floor_sheets(xlsx, n_floors)
    current_data = get_current_data(n_floors, D, mud, S, G, V, NZ, alpha, check_type)
    ...
    ParaTR = ParaTR.values[:, 1:]
    Parametri = triterazioneapp2_solotreparam(ParaTR, TrSLV)
    S_Sgeo, Cc = get_Sgeo_and_Cc(soil_category, Parametri)
    S_t = get_S_t(topographic_category)

    S_geo=S_t*S_Sgeo
    TC=Parametri[2]*Cc

    ADRS, tstep = get_ADRS(TC, Parametri, S_geo)

    passo = 5
    incr = 1.001
    deltared=0.2

    kult_TOT, Hult_TOT, vr_ult_TOT, L = compute_IR_deltau_7_fig3app(D,X,S,V,G,current_data,n_floors,NZ,Masse,alt_s,red_F,ADRS,incr,deltared,piani,alpha,passo,Parametri,ParaTR,tstep,S_geo,TC,soil_category,'add')
    
    x_coordinates, y_coordinates = calculate_bilinears(vr_ult_TOT, Hult_TOT)
    Saa, Sda, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR, ag_Tr = get_data_for_plot_2(ADRS, Parametri, Masse, vr_ult_TOT, Hult_TOT, x_coordinates, y_coordinates, kult_TOT, tstep, S_geo, TC, ParaTR, soil_category)
...
def pushover_analysis_calculation_archive(xlsx, longitude, latitude, soil_category, topographic_category, service_life, importance_class, check_type):
    country_code = country_check(latitude, longitude)
    hazard_path = 'eq_resiliance_data/HazardNTCgrid_{}.mat'.format(country_code)
    hazard_data = loadmat(hazard_path)['lonlatsism']
    ...
    kult_TOT, Hult_TOT, vr_ult_TOT, L = compute_IR_deltau_7_fig3app_archive(D,X,S,V,G,current_data,n_floors,NZ,Masse,alt_s,red_F,ADRS,incr,deltared,piani,alpha,passo,Parametri,ParaTR,tstep,S_geo,TC,soil_category,'add')
    
    x_coordinates, y_coordinates = calculate_bilinears(vr_ult_TOT, Hult_TOT)
    Saa, Sda, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR = get_data_for_plot_2(ADRS, Parametri, Masse, vr_ult_TOT, Hult_TOT, x_coordinates, y_coordinates, kult_TOT, tstep, S_geo, TC, ParaTR, soil_category)

    return vr_ult_TOT, Hult_TOT, x_coordinates, y_coordinates, ADRS, Sda, Saa, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR, L

def plot_bilinear(vr_ult_TOT, Hult_TOT):
    x_coordinates, y_coordinates = calculate_bilinears(vr_ult_TOT, Hult_TOT)
    fig = get_bilinear_pushover_plot(vr_ult_TOT, Hult_TOT, x_coordinates, y_coordinates)
    return fig
    
def plot_ADRS(ADRS, Sda, Saa, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR):
    fig, ax = plt.subplots(1, 2, figsize=(10, 7))
    directions = ['X', 'Y']
...
def global_pushover_plot(vr_ult_TOT, Hult_TOT, ADRS, Sda, Saa, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR):
    fig1 = plot_bilinear(vr_ult_TOT, Hult_TOT)
    fig2 = plot_ADRS(ADRS, Sda, Saa, delta_ult_eq, S_eq, dxstar_t, Tr, IR, ADRS_TR, Sda_TR, Saa_TR)
    figures = [fig1, fig2]
    return figures